...
public abstract class HumanoidEntityBase extends Entity implements Poseable, Geared  {
  ...
  public abstract Optional<PlayerTexture> getSkinTexture();

  @Override
  public Collection<Primitive> primitives(Vector3 offset) {
    PlayerTexture texture = switch (model) {
      case ALEX -> Texture.alex;
      default -> Texture.steve;
    };
    double armWidth = model == PlayerModel.ALEX ? 1.5 : 2;

    texture = getSkinTexture().orElse(texture);
    ...
    if (texture.getUV() instanceof PlayerTexture.ExtendedUVMap && showOuterLayer) {
      PlayerTexture.ExtendedUVMap uv = (PlayerTexture.ExtendedUVMap) texture.getUV();
  ...
  private static boolean shouldHidePlayerHead(String helmetItemId) {
    return switch (helmetItemId) {
      case "minecraft:skull", "minecraft:skeleton_skull", "minecraft:player_head", "minecraft:zombie_head",
           "minecraft:wither_skeleton_skull", "minecraft:creeper_head", "minecraft:dragon_head" -> true;
      default -> false;
    };
  }
  ...
  public static void addArmor(Collection<Primitive> faces,
    ...
    if (!headItem.isEmpty()) {
      ...
      if (headItemId.equals("minecraft:dragon_head")) {
        SkullEntity skull = new SkullEntity(new Vector3(), SkullEntity.Kind.DRAGON, 0, 1);
        ...
        SkullEntity skull = new SkullEntity(new Vector3(), SkullEntity.Kind.PIGLIN, 0, 1);
