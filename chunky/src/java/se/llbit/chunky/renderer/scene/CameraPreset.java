...
public enum CameraPreset {
  ISO_WEST_NORTH("Isometric North-West", Icon.isoNW.fxImage(), -Math.PI / 4, -Math.PI / 4) {
  ...
  ISO_EAST_SOUTH("Isometric South-East", Icon.isoSE.fxImage(), -5 * Math.PI / 4, -Math.PI / 4) {
    @Override public void apply(Camera camera) {
      camera.setView(yaw, pitch, 0);
      camera.setProjectionMode(ProjectionMode.PARALLEL);
      camera.setShift(0, 0);
