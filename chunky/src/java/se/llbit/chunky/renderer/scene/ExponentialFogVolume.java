...
public class ExponentialFogVolume extends FogVolume {
  ...
  public ExponentialFogVolume(Vector3 color, double density, double scaleHeight, double scaleOffset) {
    this.color = color;
  ...
  public boolean intersect(Ray ray, Scene scene, Random random) {
    ...
    double expYfHs = Math.exp(-(ray.o.y + scene.origin.y - scaleOffset) / scaleHeight) - expHeightDiff;
    if(expYfHs <= 0) {
      // The ray does not encounter enough fog to be scattered - no intersection.
      return false;
    }
    double yf = -Math.log(expYfHs) * scaleHeight + scaleOffset;
