...
public class RenderDump {
  ...
  @PluginApi
  public static RenderDump load(InputStream inputStream, TaskTracker taskTracker) throws IOException {
    DataInputStream dataInputStream = new DataInputStream(new BufferedInputStream(inputStream));
    DumpFormat format = readDumpFormat(dataInputStream);
    return format.load(dataInputStream, taskTracker);
  }
  ...
  private final DumpMetadata metadata;
  private final double[] sampleBuffer;

  public RenderDump(DumpMetadata metadata, double[] sampleBuffer) {
    this.metadata = metadata;
    this.sampleBuffer = sampleBuffer;
    if (sampleBuffer.length != metadata.width() * metadata.height() * 3) {
      throw new IllegalArgumentException("Sample buffer size mismatch");
    }
  }

  RenderDump(Scene scene) {
    this(new DumpMetadata(scene.canvasConfig.getWidth(), scene.canvasConfig.getHeight(), scene.spp, scene.renderTime), scene.getSampleBuffer());
  }
