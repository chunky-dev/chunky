...
public class ChiseledBookshelfTexture extends Texture {
  ...
  private boolean bookPresentAt(int x, int y) {
    if(y * 2 < empty.image.getHeight()) {
      if(x * 3 < empty.image.getWidth()) {
        return slot0;
      } else if(x * 3 / 2 < empty.image.getWidth()) {
        return slot1;
      } else {
        return slot2;
      }
  ...
  public float[] getColor(int x, int y) {
    ...
    if(shouldOverlay) {
      ColorUtil.getRGBAComponentsGammaCorrected(full.image.getPixel(x, y), result);
    } else {
      ColorUtil.getRGBAComponentsGammaCorrected(empty.image.getPixel(x, y), result);
    }
    return result;
  }
}
