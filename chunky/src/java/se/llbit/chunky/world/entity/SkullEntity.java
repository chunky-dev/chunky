...
public class SkullEntity extends Entity {
	...
	public SkullEntity(Vector3d position, CompoundTag entityTag, int metadata) {
		this(position, entityTag.get("SkullType").byteValue(0),
				entityTag.get("Rot").byteValue(0), metadata);
	}

	public SkullEntity(Vector3d position, int type, int rotation, int placement) {
		super(position);
		this.type = type;
	...
	public Collection<Primitive> primitives(Vector3d offset) {
		...
		Transform transform = Transform.NONE.translate(
				position.x + offset.x + 0.5,
				position.y + offset.y + 4/16.,
