...
public class BVH {
	...
	static class Group extends Node {
		...
		public boolean anyIntersection(Ray ray) {
			return (child1.bb.hitTest(ray) && child1.anyIntersection(ray)) ||
				(child2.bb.hitTest(ray) && child2.anyIntersection(ray));
		}
	...
	private final Comparator<Primitive> cmpX = new Comparator<Primitive>() {
		@Override
		public int compare(Primitive g1, Primitive g2) {
			AABB b1 = g1.bounds();
			AABB b2 = g2.bounds();
	...
	private final Selector selectX = new Selector() {
		@Override
		public boolean select(AABB bounds, double split) {
			double centroid = bounds.xmin + (bounds.xmax - bounds.xmin) / 2;
			return centroid < split;
		}
	};
	private final Comparator<Primitive> cmpY = new Comparator<Primitive>() {
		@Override
		public int compare(Primitive g1, Primitive g2) {
			AABB b1 = g1.bounds();
			AABB b2 = g2.bounds();
	...
	private final Selector selectY = new Selector() {
		@Override
		public boolean select(AABB bounds, double split) {
	...
	private final Comparator<Primitive> cmpZ = new Comparator<Primitive>() {
		@Override
		public int compare(Primitive g1, Primitive g2) {
			AABB b1 = g1.bounds();
			AABB b2 = g2.bounds();
	...
	private final Selector selectZ = new Selector() {
		@Override
		public boolean select(AABB bounds, double split) {
	...
	private Node constructMidpointSplit(Primitive[] primitives) {
		...
		Stack<Primitive[]> chunks = new Stack<Primitive[]>();
